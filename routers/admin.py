...
class ApproveEndTimeRequest(BaseModel):
    end_time: datetime

@notifications_router.post("/{notification_id}/approve-end-time")
async def approve_end_time(
    notification_id: int,
    data: ApproveEndTimeRequest,
    admin: models.Employee = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db)
):
    stmt = select(Notification).where(Notification.id == notification_id)
...
# Дополнительно: эндпоинт для удаления предложения (отклонение)
@notifications_router.delete("/{notification_id}/proposed-time")
...
async def monthly_report(
    ...
    # Вспомогательная функция для расчёта статистики одного дня
    def calculate_day_stats(day_entries, day_date, interval):
        ...
        # Определяем точку обрезания (если сегодня – текущее время)
        if day_date == now.date():
            day_cutoff = now if now < workday_end else workday_end
        else:
            day_cutoff = workday_end
    ...
    for emp in employees:
        ...
        # Группируем записи по рабочему дню (с учётом сдвига 5:00)
        day_map = {}
